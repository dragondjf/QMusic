...
class TreeModel(QtCore.QAbstractItemModel):
    ...
    def fetchMore(self, parent):
        ...
        if isinstance(obj, (list, tuple)):
            ...
            path_strings = ['{}[{}]'.format(obj_path, item[0]) if obj_path else
                            item[0] for item in obj_children]
        elif isinstance(obj, (set, frozenset)):
            obj_children = [('pop()', elem) for elem in sorted(obj)]
            path_strings = ['{0}.pop()'.format(obj_path, item[0]) if obj_path
                            else item[0] for item in obj_children]
            ...
            try:
                ...
                logger.info(
                    "No items expanded. Objects iteritems() call failed: {}".format(
                        ex))
            else:
                path_strings = ['{}[{!r}]'.format(obj_path, item[0]) if
                                obj_path else item[0] for item in obj_children]
        ...
        for attr_name, attr_value in sorted(inspect.getmembers(obj)):
            if ((self._show_callables or not callable(attr_value)) and
                (self._show_special_attributes or
                 not is_special_attribute(attr_name))):
                obj_children.append((attr_name, attr_value))
                path_strings.append('{}.{}'.format(obj_path, attr_name) if
                                    obj_path else attr_name)
        ...
        for item, path_str, is_attr in zip(obj_children, path_strings,
                                           is_attr_list):
            name, child_obj = item
            parent_item.append_child(self._addTreeItem(
                parent_item, child_obj, name, path_str, is_attr))
    ...
    def setShowSpecialAttributes(self, show_special_attributes):
        """ Shows/hides special attributes, which begin with an underscore.
            Repopulates the tree.
        """
        logger.debug("setShowSpecialAttributes: {}".format(
