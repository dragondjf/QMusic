...
from PyQt5.QtCore import (QObject, pyqtSignal, pyqtSlot, pyqtProperty, QDir,
                          ...
                          QThreadPool, QAbstractListModel, Qt, QModelIndex,
                          QVariant)
...
class QmlOnlineSongObject(QObject):
    ...
    def initialize(self, *agrs, **kwargs):
        self.setDict(kwargs)
        self.cover = CoverWorker.getOnlineCover(self.title, self.artist,
                                                self.album)
...
class OnlineMusicManageWorker(QObject):
        ...
        if album not in self._albums:
            if not CoverWorker.isOnlineSongCoverExisted(artist, title):
                ...
                if albumImage_100x100:
                    self.downloadOnlineSongCover.emit(artist,
                                                      title,
                                                      albumImage_100x100, )
                elif albumImage_500x500:
                    self.downloadOnlineSongCover.emit(artist, title,
                                                      albumImage_500x500)
                else:
            ...
            if not CoverWorker.isOnlineSongCoverExisted(artist, title):
                ...
                albumImage_100x100 = media['albumImage_100x100']
                if albumImage_100x100:
                    self.downloadOnlineAlbumCover.emit(
                        artist, title, albumImage_100x100, self._albums[album])
                elif albumImage_500x500:
                    self.downloadOnlineAlbumCover.emit(
                        artist, title, albumImage_500x500, self._albums[album])
                else:
                    self.downloadAlbumCover.emit(artist, album)
            else:
                self.updateSongCover(artist, title,
                                     CoverWorker.onlineSongCoverPath(artist,
                                                                     title))
    ...
    def updateSongCover(self, artist, albumtitle, coverUrl):
        ...
        for url, songObj in self._songObjs.items():
            ...
            if (songObj.title == albumtitle or
                    songObj.album == albumtitle) and songObj.artist == artist:
                cover = coverUrl
