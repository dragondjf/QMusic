...
class PosterLib(object):
    ...
    def get_album_cover(self, keywords):
        ...
        try:
            album_id = search_result.get("song_list", [{}])[0].get("album_id",
                                                                   None)
        except:
            return False
        ...
        if search_result:
            ...
            if song_lists:
                for song in song_lists:
                    title = song.get("title", "").replace("<em>", "").replace(
                        "</em>", "") or title
                    artist = song.get("author", "").replace(
                        "<em>", "").replace("</em>", "") or artist
...
def query_artist_cover_from_xiami(artist_name):
    ...
    try:
        ...
        try:
            artist_div_block = search_query(
                "div.artistBlock_list div.artist_item100_block p.buddy a.artist100 img")
