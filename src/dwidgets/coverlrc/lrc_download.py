...
class ttpClient(object):
    ...
    '''
        ...
        #EDX = EBX | Id
        #EBX = EBX | tmp3
        tmp1 = (((((tmp2 ^ tmp3) & 0x00000000FFFFFFFF) +
                  (tmp1 | Id)) & 0x00000000FFFFFFFF) *
                (tmp1 | tmp3)) & 0x00000000FFFFFFFF
...
class Engine(object):
    ...
    def tokenize(self, content):
        content = tuple((i.lower()
                         for i in re.findall("\w+", content)
                         if i.lower() not in ('a', 'an', 'the')))
        return content
    ...
    def order_results(self, results, artist, title):
        ...
        sorted_list = map(
            lambda x: x[1],
            reversed(sorted(((self.cmp_result(comp, i), i) for i in results))))
        return sorted_list
    ...
    def download(self, url):
        try:
            ...
            if self.need_check:
                if self.net_encoder == None or self.net_encoder.startswith(
                        "utf-16") or self.net_encoder.startswith("utf-32"):
...
class TTPlayer(Engine):
    ...
    def parser(self, content):
        ...
        for each_lrc in content:
            result = re.search('id=\"(.*?)\" artist=\"(.*?)\" title=\"(.*?)\"',
                               each_lrc)
            try:
                _id = result.group(1)
                ...
                download_url = self.DOWNLOAD_URL % (
                    int(_id), codes, random.randint(0, 0xFFFFFFFFFFFF))
                parser_list.append((_artist, _title, download_url,
                                    self.net_encoder))
        return parser_list

    def request(self, artist, title):
        convert_artist = ttpClient().EncodeArtTit(unicode(
            artist, self.locale).replace(u' ', '').lower())
        convert_title = ttpClient().EncodeArtTit(unicode(
            title, self.locale).replace(u' ', '').lower())
...
class SOSO(Engine):
    ...
    def parser(self, content):
        parser_list = []
        for each_lrc in content:
            result = re.search('title=\'(.*?)\'.*?title="(.*?)">', each_lrc,
                               re.S)
            try:
                _title = result.group(1)
                _artist = re.sub('<font.*?>|</font>|<strong>|</strong>', '',
                                 result.group(2))

                _url = self.SOSO_DOWNLOAD_URL % (
                    urllib.quote_plus(self.change_url_to_gb(_title)),
    ...
    def request(self, artist, title):
        url1_pre = "%s+%s" % (self.change_url_to_gb(title),
                              self.change_url_to_gb(artist))
        ...
        try:
            ...
            try:
                ...
                tmp = info_gb
            tmp_list = re.findall(r'class="title_song">.*?class="song_ablum">',
                                  tmp, re.S)
...
class DUOMI(Engine):
    ...
    def request(self, artist, title):
        ...
        try:
            artist_quote = urllib.quote_plus(unicode(
                artist, self.locale).encode(self.net_encoder))
            title_quote = urllib.quote_plus(unicode(title, self.locale).encode(
                self.net_encoder))
        except:
