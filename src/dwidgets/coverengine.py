...
from PyQt5.QtCore import (QObject, pyqtSignal, pyqtSlot, pyqtProperty, QUrl,
                          QFile, QIODevice, QTimer, QRunnable, QThread,
                          QThreadPool)
...
class NetEaseEngine(object):
    ...
    def getSongDetail(self, music_id):
        action = "http://music.163.com/api/song/detail/?id=" + str(
            music_id) + "&ids=[" + str(music_id) + "]"
        try:
    ...
    def searchCoverByArtistName(self, artist):
        ...
        try:
            ...
            picUrl = ''
            if ret['code'] == 200 and ret['result'] and ret['result'][
                    'artistCount'] >= 1:
...
class DoubanEngine(object):
    ...
    def urlread(self, url, need_gzip=True, host="www.douban.com"):
        ...
        req.add_header(
            "Cookie",
            'bid="9vSv1w9XJNs"; report=ref=%2Fsubject_search&mus_msc=musmsc_1')
    ...
    def searchCover(self, artist, album='', title=''):
        ...
        try:
            ...
            match = re.compile(self._doubanCoverPattern,
                               re.IGNORECASE).search(result)
            if match:
        ...
        try:
            if self.qtype == "artist":
                ...
                if url:
                    ...
                    if flag:
                        self.coverWorker.downloadArtistCoverSuccessed.emit(
                            self.artist, localUrl)
                    else:
                        self.coverWorker.downloadArtistCoverSuccessed.emit(
                            self.artist, '')
                else:
                ...
                if not url:
                    url = engines[1].searchCoverByAlbumName(self.artist,
                                                            self.album)
                if not url:
                    url = engines[2].searchCoverByAlbumName(self.artist,
                                                            self.album)
                if url:
                    localUrl = self.coverWorker.albumCoverPath(self.artist,
                                                               self.album)
                    flag = self.downloadCoverByUrl(url, localUrl)
                    if flag:
                        self.coverWorker.downloadAlbumCoverSuccessed.emit(
                            self.artist, self.album, localUrl)
                    else:
                        self.coverWorker.downloadAlbumCoverSuccessed.emit(
